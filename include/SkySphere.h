...
class SkySphere : public VisibleObject {
private:
...
public:
	SkySphere();
    SkySphere(glm::vec3& center, float radius);
	~SkySphere();
...
inline SkySphere::SkySphere(glm::vec3& center, float radius)
{
    p_c = center;
    r = radius;
}
...
glm::vec3 SkySphere::getIntersectInfo(const Ray& incoming) const
{
...
inline glm::vec3 SkySphere::getNormalAtPoint(glm::vec3& point) const { return (point - p_c) / r;}

inline glm::vec3 SkySphere::TextureMap(glm::vec3& point) const
{
        ...
        //# vec is the vector pointing from the SkySphere's origin (the world origin)
        //# to the point, which will also happen to be exactly equal to the SkySphere's
        //# radius.
        ...
        //# we want v to be 0 at the south pole of the SkySphere,
...
inline glm::vec3 SkySphere::getColor(glm::vec3& point) const
{
    if (mat != NULL) {
